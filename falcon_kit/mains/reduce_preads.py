...
default_sg_edges_list_file = ['sg_edges_list']
...
def run(fp_out, pread_fasta_file, sg_edges_list_file):
    ...
    if not sg_edges_list_file:
        sg_edges_list_file = default_sg_edges_list_file

    reads_in_layout = set()

    for fn in sg_edges_list_file:
...
def main(argv=sys.argv):
    ...
    parser.add_argument('--pread-fasta-file', type=str,
            default='preads4falcon.fasta',
            help='Preads file, required to construct the contigs.')
    parser.add_argument('--sg-edges-list-file', action='append',
